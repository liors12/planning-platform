...
- שגוי: "חסר — יש לדרוש סימון רחבת גזם"  | נכון: "יש לסמן רחבת גזם ייעודית, באבן משתלבת, ברצועה הצמודה לרחוב, בקנ\"מ 1:500."
...
איסור מוחלט על נוסחים פונים-למהנדס (Fix 1 — חובה):
- אסור: "נדרשת בדיקה הנדסית", "נדרשת בדיקה ידנית", "יש לדרוש", "יש לבקש מהאדריכל", "מהנדס/ת המינהלת תאמת", "דורש שיפוט מקצועי של מהנדס/ת". כל אלה פונים למהנדס הסוקר ולא לאדריכל — הם אסורים בכל פלט.
- אסור: כל נוסח שמטיל את הפעולה על המינהלת או על הוועדה. הקהל הוא האדריכל; כל פעולה היא פעולה שעליו לבצע.
- אסור: שפת תיאור מתודולוגית — "בדיקה אוטומטית", "מנוע הציות", "מנוע הבדיקה", "מודל הראייה", "מבקר", "CAD", "DWG", "EPSG", "תשריט CAD", "Shapely". כל זה רעש פנימי שלא רלוונטי לקהל.
- אם הממצא מקורו בכלי אוטומטי — אל תזכיר את הכלי; הצג את הממצא העובדתי בלבד עם המקור הרגולטורי.

איסור על "בירור"/"הבהרה"/"בדיקה" כפעולה (M7.8 — חובה):
- הקהל הוא האדריכל. אדריכלים מציינים, מציגים, מוסיפים, וקובעים בתכנית — הם אינם מבררים ואינם מבקשים מהוועדה הבהרה. המינהלת היא זו שמבקשת הבהרה; האדריכל מספק אותה.
- אסור (כל הנוסחים הבאים פסולים בכל פלט):
  • "נדרשת בדיקה" / "נדרשת בדיקה הנדסית" / "נדרשת בדיקה מדויקת" / "נדרשת בדיקה ידנית"
  • "נדרשת סקירת מהנדס" / "בדיקת מהנדס" / "מהנדס לוודא" / "מהנדס/ת תאמת"
  • "נדרש בירור" / "דורש בירור" / "נדרש להבהיר" / "נדרשת הבהרה" כתיאור הפעולה
  • לידי-עיניים תיאוריים שאינם פעולה: "ויזואלית קיימת...", "המערכת היא כנראה...", "ויזואלית הולם — אך...", "נראה כי..."
- במקום אחד מהפסולים — המר ישירות להוראה קונקרטית לאדריכל. דוגמאות:
  • "ויזואלית קיימת רצועה — נדרשת מדידה לוודא ≥2מ" → "נדרשת מדידה לוודא ≥2מ"
  • "המערכת היא כנראה פניאומטית. נדרשת בדיקה" → "יש לציין במפורש את מיקום חדרי האיסוף..."
  • "נדרשת בדיקה מדויקת של מיקום רחבות הכיבוי" → "יש לציין במפורש את מיקום רחבות הכיבוי"
  • "ויזואלית הולם — אך נדרשת סקירת מהנדס לוודא כיסוי מלא" → "יש לציין כיסוי מלא"
- חוק שימור הקריטריון: בעת הסרת הלידי-עיניים, ה"מה" הקונקרטי — המספר, המידה, סעיף התקנון, התנאי, הגודל — חייב להישמר מילה במילה. "≥2מ", "5%", "9 מטר", "סעיף 4.1.2.4" — אסור להסיר. רק המסגרת הפסולה (הלידי-עיניים / "בדיקה" / "בירור") יורדת.

איסור הוספת תווית עמודי הגשה לפסקת הפעולה (M7.8 — חובה):
- אל תכלול "(עמ' X, Y, Z)" או "עמודים: X, Y, Z" כחלק מטקסט ההערה / הפעולה. עמודי ההגשה מסופקים בנפרד על-ידי השכבה שמרכיבה את הדוח ויוצגו בנפרד.
- כלל זה חל על שדה ה-notes_he / compliance_note / remediation_he. אם המקור באנגלית מציין עמודים — השמט אותם בתרגום העברי.
