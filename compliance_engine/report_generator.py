...
# CSS lives in templates/report_chrome.css - ONE file, shared verbatim with
...
def _render_cover_with_signatures(
    ...
    # The cover markup is NOT built here - it comes from the shared chrome, so
    # the guidelines export and the דו"ח התייחסות get the identical header.
    from .report_chrome import cover_html

    return cover_html(
        title="סקירת תוכנית עיצוב",
        subtitles=[
            "תכנית בינוי ופיתוח - מתחם הטייסים-ההסתדרות",
            f"תכנית עיצוב גרסה {version} \u00b7 {sub_month_year}",
        ],
        pill=DOC_TYPE_LABEL,
        meta_rows=[
            ("תכנית סטטוטורית:", f"{plan_number} {approval_label}"),
            ("עורך התכנית:", f"אדריכלים {architect_short}"),
            ("תאריך הסקירה:", _today_he()),
        ],
        note=_COVER_STRUCTURAL_NOTE_HE,
        body_extra=f"""
